...
if is_node then
  ...
  vim.schedule(function()
    if coq then
      require('cnull.core.lsp').setup('tsserver', coq.lsp_ensure_capabilities(config))
    else
      require('cnull.core.lsp').setup('tsserver', config)
    end
  end)
...
elseif is_deno then
  ...
  vim.schedule(function()
    if coq then
      require('cnull.core.lsp').setup('denols', coq.lsp_ensure_capabilities(config))
    else
      require('cnull.core.lsp').setup('denols', config)
    end
  end)
end
